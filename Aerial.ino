...
#define LED_PIN 6
#define NUM_LEDS 144
//This line determines pulse length
#define PULSE_LENGTH 8
//These lines determine pusle color
...
#define NUM_PULSES 5
#define BACKGROUND_BRIGHTNESS 0  // 5% brightness (0-255 range)
  ...
  for (int i = 0; i < NUM_PULSES; i++) {
    if (currentMillis - pulses[i].previousMillis >= pulses[i].delay) {
      ...
      for (int j = pulses[i].position; j < pulses[i].position + PULSE_LENGTH; j++) {
        strip.setPixelColor(j, strip.Color(0, 0, BACKGROUND_BRIGHTNESS));
      }
